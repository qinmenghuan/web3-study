...
const useRewards = () => {
  ...
  const fetchPoolData = useCallback(async () => {
    ...
    try {
      ...
      setPoolData({
        poolWeight: formatUnits(pool[1] ?? 0n, 18),
        lastRewardBlock: formatUnits(pool[2] ?? 0n, 18),
        accMetaNodePerShare: formatUnits(pool[3] ?? 0n, 18),
        stTokenAmount: formatUnits(pool[4] ?? 0n, 18),
        minDepositAmount: formatUnits(pool[5] ?? 0n, 18),
        unstakeLockedBlocks: formatUnits(pool[6] ?? 0n, 18),
  ...
  const fetchRewardsData = useCallback(async () => {
    ...
    try {
      ...
      setRewardsData({
        pendingReward: formatUnits(userData[2] ?? 0n, 18),
        stakedAmount: formatUnits(stakedAmount ?? 0n, 18),
        lastUpdate: Date.now()
      });
    } catch (error) {
