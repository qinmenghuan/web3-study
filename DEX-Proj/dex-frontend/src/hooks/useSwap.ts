...
export function useSwap() {
  ...
  const getQuote = useCallback(async (params: SwapParams) => {
    ...
    try {
      ...
      if (!tokenInObj) {
        console.warn(
          `Token ${params.tokenIn} not found in local config, assuming 18 decimals`,
        );
      }
  ...
  const executeSwap = useCallback(
    async (params: SwapParams) => {
      ...
      const tokenIn = Object.values(TOKENS).find(
        (t) => t.address === params.tokenIn,
      );
      const tokenOut = Object.values(TOKENS).find(
        (t) => t.address === params.tokenOut,
      );
      ...
      const minAmountOut =
        (amountOutWei * BigInt(Math.floor((100 - params.slippage) * 100))) /
        BigInt(10000);
      ...
      const actualTokenIn =
        isNativeTokenIn && "wrappedAddress" in tokenIn
          ? (tokenIn.wrappedAddress as string)
          : params.tokenIn;

      const actualTokenOut =
        tokenOut.address === TOKENS.ETH.address &&
        "wrappedAddress" in TOKENS.ETH
